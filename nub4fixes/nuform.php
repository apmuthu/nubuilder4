...
function nuGetFormObject($F, $R, $OBJS){
		...
		while($r = db_fetch_object($t)){
			...
			if($r->sob_all_type == 'run'){
				...
				if(isProcedure($type)){
					...
					$actt			= nuRunQuery('SELECT * FROM zzzzsys_php WHERE zzzzsys_php_id = ?',Array($type));
					...
					$actt			= nuRunQuery('SELECT * FROM zzzzsys_report WHERE zzzzsys_report_id = ?',Array($type));
					$act			= db_fetch_object($actt);
	...
	$f->browse_height		= $B[1];
	$f->browse_sql			= $B[2];
	$f->browse_table_id		= nuHash();
	$f->browse_table_id		= $f->browse_table_id['TABLE_ID'];
...
function nuButtons($formid, $POST){
	...
	if($c == 'getphp' or $c == 'getreport'){
		...
		$t					= nuRunQuery($s,Array($POST['record_id']));
		$P					= db_fetch_object($t);
	...
	if($c == 'getphp'){
		...
		if($P->sph_run == 'hide'){
			return array(array('Add' => 0, 'Print' => 0, 'Save' => 0, 'Clone' => 0, 'Delete' => 0, 'Run' => '', 'RunHidden' => 'nuRunPHPHidden("'.$C.'")'), $C, $D);
		}else{
			return array(array('Add' => 0, 'Print' => 0, 'Save' => 0, 'Clone' => 0, 'Delete' => 0, 'Run' => 'nuRunPHP("'.$C.'")', 'RunHidden' => ''), $C, $D);
		}
	}
	...
	if($c == 'getreport'){
		...
		return array(array('Add' => 0, 'Print' => 0, 'Save' => 0, 'Clone' => 0, 'Delete' => 0, 'Run' => 'nuRunReport("'.$C.'")', 'RunHidden' => ''), $C, $D);
	}

	if($c != 'getphp' and $c != 'getreport'){
		return array(array('Add' => $a[0], 'Print' => $a[1], 'Save' => $a[2], 'Clone' => $a[3], 'Delete' => $a[4], 'Run' => '', 'RunHidden' => ''), $C, $D);
	}
...
// function nuFormDimensions($f){
	...
	// $browse		= Array('height'=>$bh + 0, 'width'=> $bw);
	// $edit		= Array('height'=>$h + 60,  'width'=> $w);
...
function nuGetBrowseWidth($f){
	...
	$w			= 0;
	$t			= nuRunQuery("SELECT * FROM zzzzsys_browse WHERE sbr_zzzzsys_form_id = ? ", Array($f));
